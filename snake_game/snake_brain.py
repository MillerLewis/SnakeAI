...
def simulate(snake_brains, screen_width, screen_height, video=False, wait_til_close=True):
    ...
    # for brain in brains:
    #     brain.game.reset()
        # fresh_brains.append(Brain(Game(
        #     (screen_width // 2, screen_height // 2),
        #     brain.game.snake.snake_dims,
        #     random_square(screen_width, screen_height, *brain.game.food.food_dims),
        #     brain.game.food.food_dims,
        #     (screen_width, screen_height),
        #     brain.game.tick_rate,
        #     life_time=brain.game.life_time
        # ), connections=brain.connections))

    return brains


def sim_generation(snake_brains, screen_width, screen_height, video=False, wait_til_close=True, tournament_count=2, mutate="normal", **kwargs):
...
if __name__ == "__main__":
    ...
    for generation_counter in range(GENERATIONS_TO_SIMULATE):
        ...
        snake_brains = sim_generation(snake_brains, SCREEN_WIDTH, SCREEN_HEIGHT, video=True, wait_til_close=False, tournament_count=2, mutate="normal", mean=0, std=0.1)
